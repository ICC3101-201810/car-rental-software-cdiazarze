...
namespace CarRentalSoftware
{
    class Institucion : Agrupacion
    {
        ...
        public Institucion(string miRut, float miId, int miTipo, Random rand) : base(miRut, miId, miTipo, rand)
        {
            nombre = nombres[rand.Next(0, nombres.Length)];
            edad = rand.Next(18, 80);
            autorizacion["MaquinariaPesada"] = false;
            autorizacion["Bus"] = (rand.NextDouble() >= 0.42);
